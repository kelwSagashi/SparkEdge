...
async function bootstrapSparkEdge(): Promise<void> {
  ...
  const { registerMqttCommandHandlers } =
    await import("./integrations/mqtt/mqtt.commands");
...
export class Server {
  ...
  setupMiddlewares() {
    ...
    this.app.use((req: Request, res: Response, next) => {
      ...
      if (
        protectedPrefixes.some((p) => req.path.startsWith(p)) &&
        !(req as any).user
      ) {
  ...
  public async start(port: number = 3009) {
    ...
    if (fs.existsSync(frontendPath)) {
      ...
      this.logger.log(
        `[Server] Warning: Frontend dist folder not found at ${frontendPath}`,
      );
    }
  ...
  activate(): ReturnType<typeof Router> {
    ...
    router["get"]("/health", send(handler));
