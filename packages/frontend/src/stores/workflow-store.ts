...
import type { INodeExecutionData, INodeInputConfiguration, INodeOutputConfiguration, IRun, IWorkflowBase } from 'nmg8-workflow';


type WorkflowState = {
    workflow: IWorkflowBase<INode[], Edge[]>;
    nodeClicked: INode | undefined;
    lastRun: IRun | undefined;
    isSaving: boolean;
    setNodeClicked: (node: INode | undefined) => void;
    getLastRun: () => IRun | undefined;
    setLastRun: (lastRUn: IRun | undefined) => void;
    getNodeRunData: (nodeId: string) => INodeExecutionData | undefined;
...
export const useWorkflowStore = create<WorkflowState>((set, get) => ({
    ...
    setNodeClicked: (node) => set({nodeClicked: node}),

    getLastRun: () => get().lastRun,
    
    setLastRun: (lastRun) => set({lastRun}),

    getNodeRunData: (nodeId) => get().lastRun?.data[nodeId],
